#!/usr/bin/env bash
# One-shot typed TOFU V4: offline preflight -> generation -> audit -> optional training.
# V1/V2/V3 runners and artifacts are intentionally untouched.
set -euo pipefail
...
SPLIT="${F2D_V4_SPLIT:-forget05}"
DATASET_SPLIT="${F2D_V4_DATASET_SPLIT:-${SPLIT}_perturbed}"
SEED="${F2D_V4_SEED:-42}"
UNITS=200
BLOCK_SIZE=20
...
DATA_PATH="${F2D_V4_DATA_PATH:-${EASE_ROOT}/ULD/data/ciru/${SPLIT}_author_typed${UNITS}_seed${SEED}_v4.jsonl}"
PROFILES_PATH="${F2D_V4_PROFILES_PATH:-${DATA_PATH}.profiles.json}"
STATE_DIR="${F2D_V4_STATE_DIR:-${DATA_PATH}.blocks}"
AUDIT_DIR="${F2D_V4_AUDIT_DIR:-${EASE_ROOT}/audits/${SPLIT}_author_typed${UNITS}_seed${SEED}_v4}"
...
if [ "$SPLIT" != "forget05" ]; then
    echo "Typed V4 currently requires F2D_V4_SPLIT=forget05." >&2
    exit 1
fi
...
echo "============================================================"
echo "F2D author typed V4 (exact-edit target + deterministic placebo)"
echo "  split / units      : $SPLIT / $UNITS"
echo "  immutable C11      : 10 authors x 20 TOFU QA"
echo "  C01 renderer       : atomic exact-span edits only"
echo "  C10/C00 renderer   : frozen 20-relation professional library"
echo "  semantic stage     : accept/reject judge only"
echo "  factorial JSONL    : $DATA_PATH"
echo "  resumable state    : $STATE_DIR"
echo "  legacy preservation: V1/V2/V3 untouched"
echo "============================================================"

echo "[0/3] Offline full-coverage contract preflight"
"$GEN_PY" "$EASE_ROOT/tests/test_tofu_contract_v4_full_preflight.py"

echo "[1/3] Generate typed author-level causal units"
if [ ! -s "$DATA_PATH" ]; then
    "$GEN_PY" "$EASE_ROOT/ULD/scripts/generate_tofu_author_typed_v4.py" \
        --split "$DATASET_SPLIT" \
        --manifest "$MANIFEST" \
        --output "$DATA_PATH" \
    ...
    echo "Reusing frozen typed V4 data: $DATA_PATH"
fi
...
for row in rows:
    if row.get("design_version") != "tofu-author-typed-v4":
...
if len(rows) != 200 or set(by_block) != set(expected):
    raise SystemExit("Typed V4 coverage does not equal the frozen 200-row manifest")
for block_id, block_rows in sorted(by_block.items()):
    ...
    if {r["target_entity"].casefold() for r in block_rows} != {expected[block_id].casefold()}:
        raise SystemExit(f"Block {block_id} target mismatch")
    if len({r["replacement_entity"].casefold() for r in block_rows}) != 1:
...
if profiles.get("design_version") != "tofu-author-typed-v4":
    raise SystemExit("Profile ledger is not typed V4")
print("Typed V4 hard gate OK: rows=200 blocks=10 judges=all-pass placebos=20/block")
PY
...
typed_s48_u1:2:2:16:16:1e-3:1e-3:1:1:1:1:48:48 \
typed_s60_u1:2:2:16:16:1e-3:1e-3:1:1:1:1:60:60 \
typed_s72_u1:2:2:16:16:1e-3:1e-3:1:1:1:1:72:72 \
typed_s72_u0p5:2:2:16:16:1e-3:1e-3:1:1:0.5:0.5:72:72}"
