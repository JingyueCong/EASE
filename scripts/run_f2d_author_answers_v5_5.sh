#!/usr/bin/env bash
# V5.5: frozen author ledger -> row-local complete C01 answer -> semantic audit.
set -euo pipefail
...
SPLIT="${F2D_V55_SPLIT:-forget05}"
DATASET_SPLIT="${F2D_V55_DATASET_SPLIT:-${SPLIT}_perturbed}"
SEED="${F2D_V55_SEED:-42}"
BLOCK_IDS="${F2D_V55_BLOCK_IDS:-1,4}"
GPUS="${F2D_V55_GPUS:-0 1 2 3}"
GEN_PY="${GEN_PY:-${HOME}/miniconda3/envs/ease-f2r-train/bin/python}"
MANIFEST="${F2D_V55_MANIFEST:-${EASE_ROOT}/ULD/configs/data/tofu_forget05_author_blocks.json}"
...
DATA_PATH="${F2D_V55_DATA_PATH:-${EASE_ROOT}/ULD/data/ciru/${SPLIT}_author_answers${UNITS}_seed${SEED}_v5_5_${RUN_TAG}.jsonl}"
PROFILES_PATH="${F2D_V55_PROFILES_PATH:-${DATA_PATH}.profiles.json}"
STATE_DIR="${F2D_V55_STATE_DIR:-${DATA_PATH}.blocks}"
AUDIT_DIR="${F2D_V55_AUDIT_DIR:-${EASE_ROOT}/audits/${SPLIT}_author_answers${UNITS}_seed${SEED}_v5_5_${RUN_TAG}}"
SWEEP_NAME="${F2D_V55_SWEEP_NAME:-f2d_author_answers${UNITS}_v5_5_seed${SEED}}"
DEFAULT_LEDGER_STATE="${EASE_ROOT}/ULD/data/ciru/${SPLIT}_author_slots${UNITS}_seed${SEED}_v5_4_${RUN_TAG}.jsonl.blocks"
SEED_LEDGER_STATE_DIR="${F2D_V55_SEED_LEDGER_STATE_DIR:-$DEFAULT_LEDGER_STATE}"
...
if [ "$SPLIT" != "forget05" ]; then
    echo "V5.5 currently requires F2D_V55_SPLIT=forget05." >&2
    exit 1
fi
...
SEED_ARGS=()
if [ -d "$SEED_LEDGER_STATE_DIR" ]; then
    SEED_ARGS=(--seed-ledger-state-dir "$SEED_LEDGER_STATE_DIR")
fi

echo "============================================================"
echo "F2D author ledger-answer V5.5"
echo "  split / units      : $SPLIT / $UNITS"
echo "  selected blocks    : $BLOCK_IDS"
echo "  architecture       : frozen ledger -> complete C01 answer -> row judge"
echo "  row workers/retries: $CF_ROW_CONCURRENCY / $CF_ROW_RETRIES"
echo "  profile/judge retry: $CF_PROFILE_RETRIES / $CF_JUDGE_ROUNDS"
echo "  frozen ledger state: $SEED_LEDGER_STATE_DIR"
echo "  factorial JSONL    : $DATA_PATH"
echo "  resumable state    : $STATE_DIR"
echo "  legacy preservation: FullAnswer and V1-V5.4 untouched"
echo "============================================================"

echo "[0/3] Offline 200-row anchor, ledger, slot, and answer preflight"
...
echo "[1/3] Generate frozen-ledger complete-answer causal units"
if [ ! -s "$DATA_PATH" ]; then
    "$GEN_PY" "$EASE_ROOT/ULD/scripts/generate_tofu_author_answers_v5_5.py" \
        --split "$DATASET_SPLIT" \
        --manifest "$MANIFEST" \
        --output "$DATA_PATH" \
    ...
    echo "Reusing frozen V5.5 data: $DATA_PATH"
fi
...
if len(rows) != expected_rows or len({row["source_id"] for row in rows}) != expected_rows:
    raise SystemExit("V5.5 row coverage is incomplete or duplicated")
blocks = Counter(row.get("block_id") for row in rows)
if len(blocks) != expected_blocks or set(blocks.values()) != {20}:
    raise SystemExit(f"V5.5 block coverage mismatch: {blocks}")
for row in rows:
    ...
    if row.get("render_mode") not in {
        ...
        raise SystemExit(f"Missing V5.5 render mode in {row.get('source_id')}")
...
for profile in profiles["profiles"]:
    if len(profile.get("fact_ledger", [])) != 20:
        raise SystemExit("V5.5 ledger does not cover 20 rows")
    if profile.get("profile_semantic_judge", {}).get("coherent") is not True:
        raise SystemExit("V5.5 ledger lacks semantic approval")
print(
    f"Author ledger-answer V5.5 hard gate OK: rows={len(rows)} "
    f"blocks={len(blocks)} ledgers=frozen answers=row_scoped judges=all-pass"
)
PY

echo "[3/3] V5.5 generation and audit complete"
...
if [ "$BLOCK_COUNT" -ne 10 ]; then
    echo "Refusing training on a smoke subset; rerun with F2D_V55_BLOCK_IDS=all." >&2
    exit 2
fi
...
answers_s48_u1:2:2:16:16:1e-3:1e-3:1:1:1:1:48:48 \
answers_s60_u1:2:2:16:16:1e-3:1e-3:1:1:1:1:60:60 \
answers_s72_u1:2:2:16:16:1e-3:1e-3:1:1:1:1:72:72}"
...
exec env \
    ...
    F2R_VARIANT="F2D-AuthorLedgerAnswer200-v5.5" \
