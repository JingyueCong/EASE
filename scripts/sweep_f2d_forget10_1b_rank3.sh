#!/usr/bin/env bash
# Retain-free Llama-3.2-1B Forget10 capacity experiment.
# Trains isolated rank-16/32/64 4-layer Dual Assistants, then evaluates four
# static reference-delta operating points per rank. Existing artifacts remain
# read-only and all new outputs use Forget10/1B-specific paths.
set -euo pipefail
...
FULL_AUDIT="${FULLANSWER_FORGET10_AUDIT_SUMMARY:-${EASE_ROOT}/audits/forget10_fullanswer400_seed42_incremental_v1/SUMMARY.json}"
...
RETAIN_RELATIVE="tofu_Llama-3.2-1B-Instruct_retain90/TOFU_EVAL.json"
RETAIN_LOGS_PATH="${RETAIN_LOGS_PATH:-${EASE_ROOT}/open-unlearning/saves/eval/${RETAIN_RELATIVE}}"
MODEL_ROOT="${MODEL_ROOT:-${EASE_ROOT}/ULD/outputs_trained_models/f2d_forget10_1b_rank3_seed42}"
RESULTS_DIR="${RESULTS_DIR:-${EASE_ROOT}/open-unlearning/saves/sweeps/forget10_f2d_1b_rank3_seed42}"
...
for required in "$CAUSAL_DATA" "$FULL_DATA" "$CAUSAL_AUDIT" \
    "$FULL_AUDIT" "$HUMAN_APPROVAL" "$RUNNER" "$SUMMARIZER"; do
    [ -s "$required" ] \
        || { echo "Missing Forget10 1B prerequisite: $required" >&2; exit 1; }
done
for executable in "$TRAIN_PY" "$EVAL_PY"; do
    [ -x "$executable" ] \
        || { echo "Missing Python environment: $executable" >&2; exit 1; }
done

"$TRAIN_PY" - "$CAUSAL_DATA" "$FULL_DATA" "$CAUSAL_AUDIT" \
    "$FULL_AUDIT" "$HUMAN_APPROVAL" <<'PY'
import json
import sys

causal_path, full_path, causal_audit, full_audit, approval = sys.argv[1:]
...
for label, path in (("causal", causal_audit), ("fullanswer", full_audit)):
    audit = json.load(open(path, encoding="utf-8"))
    if audit.get("records") != 400 or audit.get("unique_source_ids") != 400:
        raise SystemExit(f"{label} audit coverage gate failed")
    if audit.get("units_with_deterministic_errors") != 0:
        raise SystemExit(f"{label} deterministic audit gate failed")
    if audit.get("missing_source_indices") or audit.get("unexpected_source_indices"):
        raise SystemExit(f"{label} source-index audit gate failed")
text = open(approval, encoding="utf-8").read()
if "APPROVED for downstream training" not in text:
    raise SystemExit("Forget10 human approval gate failed")
print("forget10_1b_rank3_hard_gate rows=400 deterministic_errors=0 human_review=approved")
PY

read -r -a GPU_LIST <<< "$GPUS"
if [ "${#GPU_LIST[@]}" -lt 4 ]; then
    echo "Forget10 1B rank sweep requires four GPU ids; got: $GPUS" >&2
    exit 1
fi

cat <<EOF
============================================================
Forget10 Llama-3.2-1B static rank-capacity experiment
  ...
  A1 training    : preserve KL=.4, 192 steps, lr=5e-4
  A2 training    : preserve KL=.3, 144 steps, lr=7.5e-4
  effective batch: 16 (bs=2, ga=8)
...
if [ "$DRY_RUN" = "true" ]; then
    echo "Dry run OK; Forget10 data, audits, approval, and rank design passed."
    exit 0
fi
...
train_role() {
    ...
    if [ -n "$(checkpoint_at "$root/$role" "$steps")" ]; then
        echo "rank1_train_reuse variant=$variant role=$role rank=$rank"
        return
    fi
    echo "rank1_train_start variant=$variant role=$role rank=$rank GPU=$gpu"
    env ENV_FILE=/dev/null LOAD_DOTENV=0 MODE=full SPLIT=forget10 \
        GPU="$gpu" CF_PATH="$data" MODELS_ROOT="$root" \
        TRAIN_RUN_TAG="forget10_1b_${variant}_${role}" \
        ...
        A1_RETAIN_WEIGHT=0.4 A1_TRAIN_BS=2 A1_TRAIN_GA=8 A1_SEED=42 \
        A2_NUM_LAYER=4 A2_LORA_R="$rank" A2_LORA_ALPHA="$((2 * rank))" \
        A2_TRAIN_LR=7.5e-4 A2_TRAIN_EP=1 A2_TRAIN_STEPS=144 \
        A2_RETAIN_WEIGHT=0.3 A2_TRAIN_BS=2 A2_TRAIN_GA=8 A2_SEED=42 \
        TRAIN_MODEL_CONFIG=llama-3-1b \
        EVAL_MODEL_CONFIG=Llama-3.2-1B-Instruct_DualULD \
        HF_BASE_PREFIX=open-unlearning/tofu_Llama-3.2-1B-Instruct \
        HF_MODEL_NAME=Llama-3.2-1B-Instruct \
        ...
        F2R_VARIANT=F2D-Forget10-1B-Rank3-NoRouter \
        bash "$RUNNER" > "$TRAIN_LOGS/${variant}_${role}.log" 2>&1
    echo "rank1_train_done variant=$variant role=$role rank=$rank GPU=$gpu"
}
...
echo "[3/4] Evaluate twelve static rank/weight configurations"
if [ ! -s "$RETAIN_LOGS_PATH" ]; then
    "$EVAL_PY" - "$RETAIN_RELATIVE" "$EASE_ROOT/open-unlearning/saves/eval" <<'PY'
import sys
from huggingface_hub import snapshot_download

relative, output = sys.argv[1:]
snapshot_download(
...
POINTS=(
    "light:-1.4:1.2:0.0003"
    "balanced:-1.7:1.5:0.0003"
    "intermediate:-2.0:1.7:0.0004"
    "strong:-2.3:2.0:0.0004"
)

run_eval() {
    local gpu="$1" rank="$2" point="$3" w1="$4" w2="$5" filter="$6"
    local tag="rank${rank}_${point}"
    local task="tofu_Llama-3.2-1B-Instruct_forget10_F2R_RANK1_${tag}"
    ...
    if [ "$RESUME" = true ] && [ -s "$report" ] \
        ...
        echo "rank1_eval_reuse rank=$rank point=$point"
        return
    fi
    echo "rank1_eval_start rank=$rank point=$point GPU=$gpu"
    env ENV_FILE=/dev/null LOAD_DOTENV=0 MODE=full SPLIT=forget10 \
        ...
        A1_REFERENCE_PATH="${A1_REF[$rank]}" A2_REFERENCE_PATH="${A2_REF[$rank]}" \
        REFERENCE_PATH=auto COMPOSITION_MODE=reference_delta \
        A1_NUM_LAYER=4 A2_NUM_LAYER=4 A1_LORA_R="$rank" A2_LORA_R="$rank" \
        ...
        EVAL_MODEL_CONFIG=Llama-3.2-1B-Instruct_DualULD \
        HF_BASE_PREFIX=open-unlearning/tofu_Llama-3.2-1B-Instruct \
        HF_MODEL_NAME=Llama-3.2-1B-Instruct TASK_MODEL_NAME=Llama-3.2-1B-Instruct \
        ...
        F2R_VARIANT=F2D-Forget10-1B-Rank3-NoRouter \
        bash "$RUNNER" > "$EVAL_LOGS/${tag}.log" 2>&1
    echo "rank1_eval_done rank=$rank point=$point GPU=$gpu"
}
...
for rank in 16 32 64; do
    for spec in "${POINTS[@]}"; do
        IFS=: read -r point w1 w2 filter <<< "$spec"
        tag="rank${rank}_${point}"
        task="tofu_Llama-3.2-1B-Instruct_forget10_F2R_RANK1_${tag}"
        report="$EASE_ROOT/open-unlearning/saves/eval/$task/F2R_REPORT.json"
        echo "$tag,$w1,$w2,$filter,$task,$report,rank${rank}_seed42,$rank,${A1_PATH[$rank]},${A2_PATH[$rank]},reference_delta,true" >> "$MANIFEST"
        gpu="${GPU_LIST[$((index % ${#GPU_LIST[@]}))]}"
        run_eval "$gpu" "$rank" "$point" "$w1" "$w2" "$filter" &
        pids+=("$!")
        index=$((index + 1))
...
echo "[4/4] Summarize Forget10 1B rank-capacity results"
"$EVAL_PY" "$SUMMARIZER" --manifest "$MANIFEST" \
    --output-dir "$RESULTS_DIR" --target-agg "$TARGET_AGG" \
    --target-margin 0 --sweep-kind training \
    || failures=$((failures + 1))
echo "Forget10 1B rank table: $RESULTS_DIR/F2R_SWEEP.md"
if [ "$failures" -gt 0 ]; then
    echo "$failures Forget10 1B training/evaluation failure(s)" >&2
    exit 1
fi
