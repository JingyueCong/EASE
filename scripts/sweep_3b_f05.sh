#!/usr/bin/env bash
# Plan f05: 3B forget05 dual-ULD sweep, mirrored from Plan F/T (forget10) recipe
#   - num_layer=4, ep=10 (A1) / ep=10 (A2), lr=1e-3, r=16, bs=4 ga=4, w2=0.5
#   - retain_num=200 (1/2 of f10), rsub=forget05_k40.json
#   - retain ref: tofu_Llama-3.2-3B-Instruct_retain95
...
MODELS_DIR="${ULD_REPO}/outputs_trained_models/llama3_3b_dual_v2_f05"
RSUB="${ULD_REPO}/data/rsub/forget05_k40.json"
RETAIN_REF="${OU_REPO}/saves/eval/tofu_Llama-3.2-3B-Instruct_retain95/TOFU_EVAL.json"
REPORT_PATH="${ROOT}/uld_llama3_3b_forget05_dual_sweep.md"
FQ_LOG="/tmp/3b_f05_fq.txt"

[ -f "$RETAIN_REF" ] || { echo "ERROR: retain95 ref missing: $RETAIN_REF"; exit 1; }
...
train_a() {
    ...
    local out="${MODELS_DIR}/a${role}_forget05"
    ...
    CUDA_VISIBLE_DEVICES="$GPU" WANDB_MODE=disabled "$PY" scripts/hf_forget_train.py \
        project="llama3_3b_dual_v2_f05_a${role}_forget05" \
        ...
        trainer.strategy=gpu OUTPUTMODELDIR="$out" postfix="f05a${role}" \
        "hydra.run.dir=outputs/tune_log/llama3_3b_dual_v2_f05_a${role}_forget05/\${now:%Y-%m-%d_%H-%M-%S}"
}
...
echo "============================================================"
echo "Plan f05: 3B dual-ULD forget05 sweep"
...
# forget05: 200 retain + 200 perturbed → ~25 steps/epoch (effective bs=16)
echo ""
echo "=== Training ==="
train_a 1 10 25
train_a 2 10 25

A1_PARENT=$(find "${MODELS_DIR}/a1_forget05" -name "checkpoint-*" -type d | head -1 | xargs dirname)
A2=$(find "${MODELS_DIR}/a2_forget05" -name "checkpoint-*" -type d | awk -F'checkpoint-' '{print $NF, $0}' | sort -n | tail -1 | cut -d' ' -f2-)
...
for step in "${SCAN_STEPS[@]}"; do
    ...
    [ -d "$A1" ] || continue
    eval_dual "$A1" "$A2" -0.8 0.5 "tofu_Llama-3.2-3B-Instruct_forget05_DualULD_v2f05_a1s${step}_w1m0p8_w20p5"
done
...
for w2 in 0.3 0.7 0.8 1.0; do
    w2_str=$(echo "$w2" | tr '.' 'p')
    eval_dual "$BEST_A1" "$A2" -0.8 "$w2" "tofu_Llama-3.2-3B-Instruct_forget05_DualULD_v2f05_a1s${BEST_STEP}_w1m0p8_w2${w2_str}"
done
...
lines.append("- Recipe: Plan F/T mirror (num_layer=4, A1 ep=10 / A2 ep=10, r=16, lr=1e-3, retain_weight=5.0)")
lines.append("- Retain reference: `tofu_Llama-3.2-3B-Instruct_retain95/TOFU_EVAL.json`")
lines.append("- rsub: `forget05_k40.json`, retain_num=200")
lines.append("")
