#!/usr/bin/env bash
# V5.8: frozen core ledger -> minimal GPT C01 Q+A -> code-owned provenance.
set -euo pipefail
...
SPLIT="${F2D_V58_SPLIT:-forget05}"
DATASET_SPLIT="${F2D_V58_DATASET_SPLIT:-${SPLIT}_perturbed}"
SEED="${F2D_V58_SEED:-42}"
BLOCK_IDS="${F2D_V58_BLOCK_IDS:-1,4}"
GPUS="${F2D_V58_GPUS:-0 1 2 3}"
GEN_PY="${GEN_PY:-${HOME}/miniconda3/envs/ease-f2r-train/bin/python}"
MANIFEST="${F2D_V58_MANIFEST:-${EASE_ROOT}/ULD/configs/data/tofu_forget05_author_blocks.json}"
...
DATA_PATH="${F2D_V58_DATA_PATH:-${EASE_ROOT}/ULD/data/ciru/${SPLIT}_author_semantic${UNITS}_seed${SEED}_v5_8_${RUN_TAG}.jsonl}"
PROFILES_PATH="${F2D_V58_PROFILES_PATH:-${DATA_PATH}.profiles.json}"
STATE_DIR="${F2D_V58_STATE_DIR:-${DATA_PATH}.blocks}"
AUDIT_DIR="${F2D_V58_AUDIT_DIR:-${EASE_ROOT}/audits/${SPLIT}_author_semantic${UNITS}_seed${SEED}_v5_8_${RUN_TAG}}"
SWEEP_NAME="${F2D_V58_SWEEP_NAME:-f2d_author_semantic${UNITS}_v5_8_seed${SEED}}"
PREFLIGHT_LOG="${F2D_V58_PREFLIGHT_LOG:-${STATE_DIR}.preflight.log}"
...
if [ "$SPLIT" != "forget05" ]; then
    echo "V5.8 currently requires F2D_V58_SPLIT=forget05." >&2
    exit 1
fi
...
echo "============================================================"
echo "F2D author semantic-contrast V5.8"
echo "  split / units      : $SPLIT / $UNITS"
echo "  selected blocks    : $BLOCK_IDS"
echo "  architecture       : frozen ledger -> minimal GPT C01 Q+A -> judge"
echo "  generator output   : c01_question + replacement_answer only"
echo "  provenance         : deterministic sequence diff (audit only)"
...
echo "  legacy preservation: FullAnswer and V1-V5.7 untouched"
echo "============================================================"

echo "[0/3] Offline legacy and semantic-interface preflight"
...
echo "[1/3] Generate minimal semantic GPT C01 interventions"
if [ ! -s "$DATA_PATH" ] || [ ! -s "$PROFILES_PATH" ]; then
    "$GEN_PY" "$EASE_ROOT/ULD/scripts/generate_tofu_author_semantic_v5_8.py" \
        --split "$DATASET_SPLIT" \
        --manifest "$MANIFEST" \
        --output "$DATA_PATH" \
    ...
    echo "Reusing frozen V5.8 data: $DATA_PATH"
fi

echo "[2/3] Deterministic schema and semantic hard gate"
AUDIT_EXPECTED=()
...
design = "tofu-author-semantic-contrast-v5.8"
renderer = "semantic-complete-c01-question-answer-v5.8"
mapping = "row-local-semantic-question-answer-conditioned-on-frozen-ledger"
schema = "semantic-question-answer-core-v1"
contract_policy = "semantic-joint-compatible-v5.8"
provenance_method = "deterministic-sequence-diff-v1"
output_fields = ["c01_question", "replacement_answer"]
judge_fields = (
...
if len(rows) != expected_rows or len({row["source_id"] for row in rows}) != expected_rows:
    raise SystemExit("V5.8 row coverage is incomplete or duplicated")
blocks = Counter(row.get("block_id") for row in rows)
if len(blocks) != expected_blocks or set(blocks.values()) != {20}:
    raise SystemExit(f"V5.8 block coverage mismatch: {blocks}")
for row in rows:
    ...
    if generation.get("generator_output_fields") != output_fields:
        raise SystemExit(f"Generator output contract drift in {source_id}")
    ...
    if provenance.get("method") != provenance_method:
        raise SystemExit(f"Wrong provenance method in {source_id}")
    if provenance.get("validation_role") != "audit_only":
        raise SystemExit(f"Provenance became an acceptance gate in {source_id}")
    if not isinstance(provenance.get("operations"), list):
        raise SystemExit(f"Missing deterministic question diff in {source_id}")
    for operation in provenance["operations"]:
        if operation.get("operation") not in {"replace", "delete", "insert"}:
            raise SystemExit(f"Invalid question diff operation in {source_id}")
        if not isinstance(operation.get("source_span"), list):
            raise SystemExit(f"Missing source span in {source_id}")
        if not isinstance(operation.get("replacement_span"), list):
            raise SystemExit(f"Missing replacement span in {source_id}")
    if any(row.get("semantic_judge", {}).get(field) is not True for field in judge_fields):
        raise SystemExit(f"Unapproved semantic row {source_id}")

if profiles.get("design_version") != design:
    raise SystemExit("Profile artifact is not semantic-contrast V5.8")
if profiles.get("contrast_schema_version") != schema:
    raise SystemExit("Profile artifact has the wrong V5.8 schema")
if profiles.get("generator_output_fields") != output_fields:
    raise SystemExit("Profile artifact has the wrong generator output contract")
if profiles.get("provenance_method") != provenance_method:
    raise SystemExit("Profile artifact has the wrong provenance method")
if len(profiles.get("profiles", [])) != expected_blocks:
    raise SystemExit("V5.8 profile block count mismatch")
for profile in profiles["profiles"]:
    if len(profile.get("fact_ledger", [])) != 20:
        raise SystemExit("V5.8 ledger does not cover 20 rows")
    verdict = profile.get("profile_semantic_judge", {})
    if verdict.get("coherent") is not True or verdict.get("contrastive") is not True:
        raise SystemExit("V5.8 profile lacks independent contrast approval")
print(
    f"Author semantic-contrast V5.8 hard gate OK: rows={len(rows)} "
    f"blocks={len(blocks)} model_fields=2 provenance=code-owned"
)
PY

echo "[3/3] V5.8 generation and audit complete"
...
if [ "$BLOCK_COUNT" -ne 10 ]; then
    echo "Refusing training on a smoke subset; rerun with F2D_V58_BLOCK_IDS=all." >&2
    exit 2
fi
...
semantic_s48_u1:2:2:16:16:1e-3:1e-3:1:1:1:1:48:48 \
semantic_s60_u1:2:2:16:16:1e-3:1e-3:1:1:1:1:60:60 \
semantic_s72_u1:2:2:16:16:1e-3:1e-3:1:1:1:1:72:72}"
...
exec env \
    ...
    F2R_VARIANT="F2D-AuthorSemanticContrast200-v5.8" \
