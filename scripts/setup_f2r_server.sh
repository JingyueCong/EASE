...
"$CONDA_BIN" run -n "$TRAIN_ENV" python -m pip install \
    ...
    accelerate==0.34.2 bitsandbytes==0.43.1 deepspeed==0.14.2 \
...
"$CONDA_BIN" run -n "$EVAL_ENV" python -m pip install \
    -r "$EASE_ROOT/open-unlearning/requirements.txt" peft==0.15.2
...
"$CONDA_BIN" run -n "$TRAIN_ENV" python -c \
    'import numpy,torch,transformers,tokenizers,datasets,peft,accelerate,pkg_resources; from accelerate.utils.memory import clear_device_cache; a=torch.from_numpy(numpy.zeros(1)); print("train:", torch.__version__, transformers.__version__, "tokenizers=", tokenizers.__version__, "peft=", peft.__version__, "accelerate=", accelerate.__version__, "numpy=", numpy.__version__, "cuda=", torch.cuda.is_available())'
