#!/usr/bin/env bash
# V5.4: frozen ledger -> one local edit list -> deterministic row rendering.
set -euo pipefail
...
SPLIT="${F2D_V54_SPLIT:-forget05}"
DATASET_SPLIT="${F2D_V54_DATASET_SPLIT:-${SPLIT}_perturbed}"
SEED="${F2D_V54_SEED:-42}"
BLOCK_IDS="${F2D_V54_BLOCK_IDS:-1,4}"
GPUS="${F2D_V54_GPUS:-0 1 2 3}"
GEN_PY="${GEN_PY:-${HOME}/miniconda3/envs/ease-f2r-train/bin/python}"
MANIFEST="${F2D_V54_MANIFEST:-${EASE_ROOT}/ULD/configs/data/tofu_forget05_author_blocks.json}"
...
DATA_PATH="${F2D_V54_DATA_PATH:-${EASE_ROOT}/ULD/data/ciru/${SPLIT}_author_slots${UNITS}_seed${SEED}_v5_4_${RUN_TAG}.jsonl}"
PROFILES_PATH="${F2D_V54_PROFILES_PATH:-${DATA_PATH}.profiles.json}"
STATE_DIR="${F2D_V54_STATE_DIR:-${DATA_PATH}.blocks}"
AUDIT_DIR="${F2D_V54_AUDIT_DIR:-${EASE_ROOT}/audits/${SPLIT}_author_slots${UNITS}_seed${SEED}_v5_4_${RUN_TAG}}"
SWEEP_NAME="${F2D_V54_SWEEP_NAME:-f2d_author_slots${UNITS}_v5_4_seed${SEED}}"
DEFAULT_V53_STATE="${EASE_ROOT}/ULD/data/ciru/${SPLIT}_author_ledger${UNITS}_seed${SEED}_v5_3_${RUN_TAG}.jsonl.blocks"
SEED_V53_STATE_DIR="${F2D_V54_SEED_V53_STATE_DIR:-$DEFAULT_V53_STATE}"
...
if [ "$SPLIT" != "forget05" ]; then
    echo "V5.4 currently requires F2D_V54_SPLIT=forget05." >&2
    exit 1
fi
...
SEED_ARGS=()
if [ -d "$SEED_V53_STATE_DIR" ]; then
    SEED_ARGS=(--seed-v53-state-dir "$SEED_V53_STATE_DIR")
fi

echo "============================================================"
echo "F2D author ledger-slots V5.4"
echo "  split / units      : $SPLIT / $UNITS"
echo "  selected blocks    : $BLOCK_IDS"
echo "  architecture       : frozen ledger -> local edit list -> deterministic render"
echo "  row workers/retries: $CF_ROW_CONCURRENCY / $CF_ROW_RETRIES"
echo "  profile/judge retry: $CF_PROFILE_RETRIES / $CF_JUDGE_ROUNDS"
echo "  V5.3 seed state    : $SEED_V53_STATE_DIR"
echo "  factorial JSONL    : $DATA_PATH"
echo "  resumable state    : $STATE_DIR"
echo "  legacy preservation: FullAnswer and V1-V5.3 untouched"
echo "============================================================"

echo "[0/3] Offline 200-row anchor, ledger, and local-slot preflight"
...
echo "[1/3] Generate frozen-ledger local-slot causal units"
if [ ! -s "$DATA_PATH" ]; then
    "$GEN_PY" "$EASE_ROOT/ULD/scripts/generate_tofu_author_slots_v5_4.py" \
        --split "$DATASET_SPLIT" \
        --manifest "$MANIFEST" \
        --output "$DATA_PATH" \
    ...
    echo "Reusing frozen V5.4 data: $DATA_PATH"
fi
...
design = "tofu-author-ledger-slots-v5.4"
renderer = "deterministic-ledger-local-slot-v5.4"
mapping = "frozen-ledger-conditioned-local-slot-edits"
judge_fields = (
...
if len(rows) != expected_rows or len({row["source_id"] for row in rows}) != expected_rows:
    raise SystemExit("V5.4 row coverage is incomplete or duplicated")
blocks = Counter(row.get("block_id") for row in rows)
if len(blocks) != expected_blocks or set(blocks.values()) != {20}:
    raise SystemExit(f"V5.4 block coverage mismatch: {blocks}")
for row in rows:
...
if profiles.get("design_version") != design:
    raise SystemExit("Profile artifact is not ledger-slots V5.4")
if len(profiles.get("profiles", [])) != expected_blocks:
    raise SystemExit("V5.4 profile block count mismatch")
for profile in profiles["profiles"]:
    if len(profile.get("fact_ledger", [])) != 20:
        raise SystemExit("V5.4 ledger does not cover 20 rows")
    if profile.get("profile_semantic_judge", {}).get("coherent") is not True:
        raise SystemExit("V5.4 ledger lacks semantic approval")
    if profile.get("reconciliation_conflicts"):
        raise SystemExit("V5.4 must not have cross-row anchor reconciliation")
print(
    f"Author ledger-slots V5.4 hard gate OK: rows={len(rows)} "
    f"blocks={len(blocks)} ledgers=frozen local_slots=row_scoped judges=all-pass"
)
PY

echo "[3/3] V5.4 generation and audit complete"
...
if [ "$BLOCK_COUNT" -ne 10 ]; then
    echo "Refusing training on a smoke subset; rerun with F2D_V54_BLOCK_IDS=all." >&2
    exit 2
fi
...
slots_s48_u1:2:2:16:16:1e-3:1e-3:1:1:1:1:48:48 \
slots_s60_u1:2:2:16:16:1e-3:1e-3:1:1:1:1:60:60 \
slots_s72_u1:2:2:16:16:1e-3:1e-3:1:1:1:1:72:72}"
...
exec env \
    ...
    F2R_VARIANT="F2D-AuthorLedgerSlots200-v5.4" \
